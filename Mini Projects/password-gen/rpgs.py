...
def pass_save(pswd,a_name):
    ...
    save pswd
    '''
    with open(f"D:\\Python-MiniProjects\\Mini Projects\\password-gen\\{a_name}.txt","a") as p:
        p.write(pswd)
...
def pass_access():
    ...
    with open(f"D:\\Python-MiniProjects\\Mini Projects\\password-gen\\{ac_fname}.txt","r") as f:
        f.seek(0)
        ac_pswd = f.readline()
...
def pass_delete():
    ...
    with open(f"D:\\Python-MiniProjects\\Mini Projects\\password-gen\\{d_fname}.txt","r") as d:
        d.seek(0)
        d_pswd = d.readline()
    print(f"Saved password is: {d_pswd}")
    dec = input("Are you sure you want to delete this password?(yes/no)\n")
    if dec.lower()=='yes':
        os.remove(f"D:\\Python-MiniProjects\\Mini Projects\\password-gen\\{d_fname}.txt")
